...
Manter a evolução da NEXUS dentro de limites explícitos, auditáveis e reversíveis. Este arquivo é a fonte canônica dos bloqueios de liberação da PR `audit/premium-elite-v1`.
...
|---|---|---:|---|---|---|
| CTRL-001 | CI não executado ou não visível | crítica | aberto | nenhum status no SHA da PR | habilitar/acionar Actions e obter execução aprovada |
| CTRL-002 | branch divergente da `main` | alta | aberto | branch à frente e atrás da base | inventariar commits exclusivos antes de merge/rebase |
...
|---|---|---|
| G1 — CI | execução visível e verde no SHA atual | bloqueado |
| G2 — Integridade | validador, links, IDs e secret scan aprovados | aguardando CI |
| G3 — Testes | todos os autotestes Python e TypeScript aprovados | aguardando CI |
| G4 — Sincronização | branch alinhada à `main` sem perda de histórico | bloqueado |
