...
| AUD-P2-003 | P2 | `examples/observability_pipeline.py` | redigir segredos embutidos em valores permitidos | self-test de observabilidade | ADR-002; REF-003; REF-005 | nenhum segredo sintético persistido | 14/14 no baseline corretivo |
| AUD-P2-004 | P2 | quarentena de telemetria | aplicar sanitização antes da quarentena | teste adversarial de schema incompatível com segredo | ADR-002 | quarentena sem dado sensível | obrigatório antes de readiness final |
...
- `examples/observability_pipeline.py --self-test`;
