...
| AUD-P2-008 | P2 | buffer e deduplicação | remover o estado de deduplicação de evento comum efetivamente evicto por evento crítico | `test_evicted_event_can_be_retried_instead_of_becoming_false_duplicate` | ADR-002 | retry posterior não retorna falso `duplicate` | implementado na branch corretiva; aguarda CI |
| AUD-P2-009 | P2 | alertas operacionais | normalizar owner de e-mail sintético para identificador local e rejeitar owner sensível não acionável | `test_alert_owner_remains_operational_and_sensitive_owner_is_rejected` | ADR-002; REF-005 | alerta armazenado possui owner operacional não sensível | implementado na branch corretiva; aguarda CI |
...
| evento comum evicto por crítico | deduplicação | retry legítimo permanece possível |
| owner de alerta com e-mail sintético | alerts | identificador operacional local preservado sem domínio |
| owner contendo segredo | alerts | alerta rejeitado antes de armazenamento |
| runbook com credencial em URL | alerts | credencial redigida e referência continua acionável |
...
- `tests/test_pr8_review_findings.py`;
...
A matriz registra cobertura conhecida. Ela não equivale a prova de ausência universal de regressões ou vazamentos. Novos formatos de segredo, mutações externas indevidas das estruturas públicas de buffer e dependências externas exigem revisão contínua.
